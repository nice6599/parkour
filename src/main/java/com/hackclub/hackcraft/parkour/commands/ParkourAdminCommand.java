...
public class ParkourAdminCommand implements CommandExecutor {
  ...
  public boolean onCommand(CommandSender commandSender, Command command, String label,
    ...
    if (args[0].equalsIgnoreCase("details")) {
      ...
      sender.sendMessage(
          ChatColor.AQUA + "Map \"" + ChatColor.GREEN + pm.get().getName() + ChatColor.AQUA
              + "\": ");
      ...
      for (int i = 0; i < pm.get().getCheckpoints().size(); i++) {
        ...
        if (l.distance(sender.getLocation()) < 5) {
          sender.sendMessage(
              ChatColor.GREEN + String.valueOf(i) + ": " + ChatColor.BLUE + Util.locationToChat(l)
                  + ChatColor.GREEN + " (You are next to this one!)");
          continue;
        }
    ...
    if (args[0].equalsIgnoreCase("rmch")) {
      ...
      if (pm2.removeCheckpoint(Integer.parseInt(args[2])) && plugin.parkourUtil
          .saveParkourMap(pm2)) {
