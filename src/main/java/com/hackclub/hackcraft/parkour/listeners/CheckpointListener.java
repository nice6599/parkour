...
public class CheckpointListener implements Listener {
    ...
    public void onCheckpoint(CheckpointEvent e) {
        e.getPlayer().playSound(e.getPlayer().getLocation(), Sound.BLOCK_NOTE_BLOCK_CHIME, 1.0F,
                8.0F);
        pl.getDataManager().getData(e.getPlayer()).getCurrentRun()
                .setLastCheckpoint(e.getCheckpointIndex());
    }
}
